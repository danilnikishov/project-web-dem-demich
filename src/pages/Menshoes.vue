...
const fetchItems = async () => {
  try {
    ...
    const filtered = !filters.category
      ? data.filter((item) => item.category === 'menwinter' || item.category === 'mensport')
      : data // иначе сервер вернул только выбранную категорию
...
<template>
  <div class="px-4 sm:px-6 md:px-8 lg:px-10">
    ...
    >
      <h2 class="text-2xl sm:text-3xl md:text-4xl font-bold">Вся мужская обувь</h2>
      ...
      <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto">
        ...
        <div class="flex gap-3 sm:gap-2 w-full sm:w-auto">
          ...
          >
            ...
            <option value="menwinter">Зимняя обувь</option>
            <option value="mensport">Спортивные кроссовки</option>
          </select>
        </div>
      </div>
    </div>
